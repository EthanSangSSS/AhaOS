...
from ahaos.config import get_tag_keywords, load_config
from ahaos.models import MemoryAtom, InsightCandidate
...
def save_pilot_state(
    ...
    state = {
        ...
        # P2-B: candidate maturation history
...
def extract_structural_signature(claim: str) -> dict[str, str | None]:
    ...
    # P2-C: trigger condition slot
    trig_match = re.search(r"\b(when|if|after|before|upon|on failure)\b\s*(\w+(?:\s+\w+){0,3})", lowered)
    slots["trigger_condition"] = trig_match.group(0) if trig_match else None

    # P2-C: target artifact slot (workflow-level assets)
    art_match = re.search(r"\b(workflow|pipeline|checklist|playbook|runbook|process|procedure|template)\b", lowered)
    slots["target_artifact"] = art_match.group(0) if art_match else None

    # P2-C: outcome slot
    out_match = re.search(r"\b(result|output|produces?|generates?|creates?|returns?|handoff|deploy)\b\s*(\w+(?:\s+\w+){0,3})", lowered)
    slots["outcome"] = out_match.group(0) if out_match else None

    # P2-C: frequency slot
    freq_match = re.search(r"\b(always|every|each|recurring|repeated|pattern|multiple|again|second|another)\b", lowered)
    slots["frequency"] = freq_match.group(0) if freq_match else None
...
def promote_pilot_candidate(c_dict: dict[str, Any], atoms_by_id: dict[str, MemoryAtom], activation_count: int = 0) -> InsightCandidate:
    # Resolve evidence from source_atom_ids
    evidence = []
    for aid in c_dict.get("source_atom_ids", []):
    ...
    return InsightCandidate(
        ...
        actionability=0.7,
        evidence_strength=min(1.0, len(deduped_evidence) / 3),
        risk=0.2,
        surprise=float(c_dict.get("structural_distance", 0.5)),
        relevance=0.6,
        activation_count=activation_count,
        recommended_action=c_dict.get("next_smallest_check", ""),
        do_not_do=c_dict.get("boundary", "Do not take action without confirming baseline logic."),
    )
...
def run_pilot(
    ...
    candidates = incubate(
        ...
        loaded_loops,
        current_time=now,
        trigger_ids=trigger_atom_ids,
        candidate_history=state_data.get("candidate_history", {}),
    )

    # Multi-cycle candidate promotion: promote matured pilot candidates
    atoms_by_id = {atom.id: atom for atom in loaded_atoms}
    promoted_cands = []
    history = state_data.get("candidate_history", {})
    for c_dict in cands:
        count = int(history.get(c_dict["id"], {}).get("seen_count", 0))
        if count >= 2:
            promoted_cands.append(promote_pilot_candidate(c_dict, atoms_by_id, activation_count=count))
    candidates.extend(promoted_cands)

    # Deduplicate candidate list by id
    unique_candidates = {}
    for c in candidates:
    ...
    # P2-B: update candidate_history for maturation tracking
    candidate_history: dict[str, Any] = dict(state_data.get("candidate_history", {}))
    for c in candidates:
        prev = candidate_history.get(c.id, {})
        candidate_history[c.id] = {
            "first_seen_at": prev.get("first_seen_at", now),
            "seen_count": int(prev.get("seen_count", 0)) + 1,
            "last_score": round(c.score, 4),
        }

    save_pilot_state(work_dir, atoms, loops, now, candidate_history)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--work-dir", default=Path("/tmp/ahaos-work"), type=Path)

...
def verify_candidate(candidate: InsightCandidate) -> tuple[bool, list[str]]:
    errors: list[str] = []
    ...
    if all(item.trust_level == "model_inference" for item in candidate.evidence):
        errors.append("model inference cannot be the only evidence")

    return not errors, errors


def filter_verified(candidates: list[InsightCandidate]) -> list[InsightCandidate]:
    verified: list[InsightCandidate] = []
    for candidate in candidates:
        ok, _ = verify_candidate(candidate)
        if ok:

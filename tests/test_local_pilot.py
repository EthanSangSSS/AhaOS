...
from scripts.local_pilot import (
...
def test_run_local_pilot_writes_memory_and_report(tmp_path: Path) -> None:
    ...
    result = run_local_pilot(input_dir, work_dir, report_dir, top_k=3, dry_run=False)
...
def test_candidates_and_latent_links_generation(tmp_path: Path) -> None:
    ...
    result = run_local_pilot(input_dir, work_dir, report_dir, top_k=3, dry_run=True)
...
def test_local_pilot_state_and_trigger_and_structural_fields(tmp_path: Path) -> None:
    input_dir = tmp_path / "input"
    work_dir = tmp_path / "work"
    report_dir = tmp_path / "reports"
    input_dir.mkdir()
    ...
    run_local_pilot(input_dir, work_dir, report_dir, top_k=3, dry_run=True)
